...
const ProjectHeader: React.FC<ProjectHeaderProps> = ({
  ...
  const [subElectricalConsultants, setSubElectricalConsultants] = useState<User[]>([]);
  const [subArchitectConsultants, setSubArchitectConsultants] = useState<User[]>([]);
  const [subMechanicalConsultants, setSubMechanicalConsultants] = useState<User[]>([]);

  // اختيارات المستخدم
  const [selectedOwnerId, setSelectedOwnerId] = useState(project.ownerId || "");
  const [selectedContractorId, setSelectedContractorId] = useState(project.contractorId || "");
  const [selectedElectricalConsultant, setSelectedElectricalConsultant] = useState((project as any).electricalConsultantId || "");
  const [selectedArchitectConsultant, setSelectedArchitectConsultant] = useState((project as any).architectConsultantId || "");
  const [selectedMechanicalConsultant, setSelectedMechanicalConsultant] = useState((project as any).mechanicalConsultantId || "");
  ...
  useEffect(() => {
    const loadUserNames = async () => {
      ...
      try {
        ...
        if ((project as any).subcontractor_id)
          names.subcontractor = subcontractors.find(s => s.id === (project as any).subcontractor_id)?.name || "غير معروف";

        if ((project as any).electricalConsultantId)
          names.electricalConsultant = await getUserNameById((project as any).electricalConsultantId);
        if ((project as any).architectConsultantId)
          names.architectConsultant = await getUserNameById((project as any).architectConsultantId);
        if ((project as any).mechanicalConsultantId)
          names.mechanicalConsultant = await getUserNameById((project as any).mechanicalConsultantId);
    ...
    setSelectedElectricalConsultant((project as any).electricalConsultantId || "");
    setSelectedArchitectConsultant((project as any).architectConsultantId || "");
    setSelectedMechanicalConsultant((project as any).mechanicalConsultantId || "");
  ...
  const handleSaveAssignments = async () => {
    ...
    if (success) {
      const names = {
        owner: await getUserNameById(selectedOwnerId),
        contractor: await getUserNameById(selectedContractorId),
        electricalConsultant: await getUserNameById(selectedElectricalConsultant),
        architectConsultant: await getUserNameById(selectedArchitectConsultant),
        mechanicalConsultant: await getUserNameById(selectedMechanicalConsultant),
      };
      setUserNames(prev => ({ ...prev, ...names }));
    } else {
      console.error("فشل في الحفظ.");
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="p-4 sm:p-6 shadow-sm border-0 bg-white">
        {canEdit && (
          <>
            {/* أول صف - استشاريين فرعيين حسب النوع */}
            <div className="grid md:grid-cols-3 gap-4 mb-6">
              {/* استشاري كهرباء فرعي */}
              <div>
                <label className="block text-sm font-medium text-gray-600 mb-1">
