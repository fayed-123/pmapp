...
class AuthService {
 async login(nameOrEmail: string, role: string): Promise<User | null> {
  try {
    // 👇 تعديل الدور قبل ما نبعته
    const actualRole = role === "subcontractor" ? "contractor" : role;
    ...
    const { data: user, error } = await supabase
      ...
      .eq('role', actualRole) // 👈 استخدم الدور المعدل هنا
      .single();
 ...
 async register(userData: Omit<User, 'id' | 'approved'>): Promise<boolean> {
  try {
    ...
    const actualRole = userData.role === "subcontractor" ? "contractor" : userData.role;
